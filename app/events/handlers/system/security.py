...
def handle_user_login_attempted(event: UserLoginAttempted) -> None:
...
def handle_password_change(event: UserPasswordChanged) -> None:
...
def handle_account_locked(event: UserAccountLocked) -> None:
...
def handle_suspicious_activity(event: SuspiciousLoginActivity) -> None:
...
def register_security_handlers() -> None:
    ...
    shared_event_bus.subscribe(UserLoginAttempted, handle_user_login_attempted)
    shared_event_bus.subscribe(UserPasswordChanged, handle_password_change)
    shared_event_bus.subscribe(UserAccountLocked, handle_account_locked)
    shared_event_bus.subscribe(SuspiciousLoginActivity, handle_suspicious_activity)
